	...
	<head>
		...
		<script>
			function showTableLotti(id)
			{
				...
				xmlhttp.open("POST", "tabellaLotti.php?commessa=" + commessa, true);
				xmlhttp.send();
			}
			...
			function genera(n,id)
			{
				...
				xmlhttp.open("POST", "generaExcelLotti.php?id_lotto=" + id, true);
			...
			function svuotaLottoCorrenteTimeout()
			{
				...
				xmlhttp.open("POST", "svuotaLottoCorrenteTimeout.php?", true);
				xmlhttp.send();
			}
			function modifica(riga,colonna,id)
			{
				...
				xmlhttp.open("POST", "aggiornaNote.php?nomeColonna=" + nomeColonna + "&valore=" + valore + "&id=" + id, true);
				xmlhttp.send();
			}
			function erroreGenerabile(lotto,commessa)
			{
				...
				xmlhttp.open("POST", "erroreGenerabile.php?lotto=" + lotto + "&commessa=" + commessa, true);
				xmlhttp.send();
			}
			function controlloIntegritaLotti()
			{
				...
				$.post("controlloIntegritaLotti.php",
				function(response, status)
				{
					if(status=="success")
					{
						if(response.indexOf("ok")>-1)
						{
							...
							({
								type: 'success',
								title: 'Controllo integrita lotti completato'
							})
							showTableLotti(0);
							removeCircleSpinner();
						}
						else
						{
							Swal.fire({
							  type: 'error',
							  title: 'Errore',
							  text: "Impossibile controllare l'integrita dei lotti"
							})
						}
					}
					else
