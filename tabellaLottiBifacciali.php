<?php
	...
	echo '</tr>';
		...
		$queryRighe="SELECT lotti_bf.*,commesse.commessa FROM lotti_bf,commesse WHERE lotti_bf.commessa=commesse.id_commessa AND commesse.commessa LIKE '$commessa' ORDER BY lotto DESC";//echo $queryRighe;
			...
			while($rowRighe=sqlsrv_fetch_array($resultRighe))
			{
				echo '<tr class="myTableLottiRowNormal" >';
					...
					echo "<td style='text-align:center;width:150px;max-width:150px;'>";
						...
						{
							echo '<form action="downloadExcelLotti_bf.php" method="POST" id="downloadForm'.$rowRighe['id_lotto'].'" name="downloadForm'.$rowRighe['id_lotto'].'" style="height:20px;display:inline-block;">';
							...
							if($rowRighe['nVersioni']>1)
							{
								...
								echo "<div id='versioni".$rowRighe['id_lotto']."' class='versioni'>";
									...
									{
										echo '<form action="downloadExcelLotti_bf.php" method="POST" style="height:10px;display:inline-block;">';
