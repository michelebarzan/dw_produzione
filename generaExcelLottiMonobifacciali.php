<?php
	...
	if(controllaLotto_corrente($conn))
	{
		...
		//Cancello file excel
		exec('del "C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\SCHEDA_PRODUZIONE_mb.xlsx"');
		
		//faccio partire excel 
		exec( '"C:\\Program Files (x86)\\Microsoft Office\\Office15\\EXCEL.EXE" "C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\SCHEDA_PRODUZIONE_mb.xlsm"');
		
		//Copio il file excel e lo rinomino
		exec('copy "C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\SCHEDA_PRODUZIONE_mb.xlsx" "C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\'.$id_lotto.'SCHEDA_PRODUZIONE_'.$commessa.'_'.$lotto.'_Versione'.$nVersione.'.xlsx"');
		//Cancello file excel
		exec('del "C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\SCHEDA_PRODUZIONE_mb.xlsx"');
		
		//faccio partire access 
		exec( '"C:\\xampp\\htdocs\\dw_produzione\\files\\lotti\\Etichette_mb.accdb"');
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		$query="UPDATE lotti_mb SET avvisoRevisioneCodcab='false' WHERE id_lotto=$id_lotto";
	...
	{
		$query="UPDATE lotti_mb SET nVersioni=nVersioni+1 WHERE id_lotto=$id_lotto";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="SELECT nVersioni FROM lotti_mb WHERE id_lotto=$id_lotto";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="UPDATE lotti_mb SET ultimaVersione=0 WHERE id_lotto=$id_lotto";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="SELECT commesse.commessa FROM lotti_mb,commesse WHERE lotti_mb.commessa=commesse.id_commessa AND id_lotto=$id_lotto";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="SELECT lotto FROM lotti_mb WHERE id_lotto=$id_lotto";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="INSERT INTO Lotto_corrente_mb (id_lotto,id_commessa,lotto,commessa) VALUES ($id_lotto,$id_commessa,$lotto,'$commessa')";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="DELETE Lotto_corrente_mb FROM Lotto_corrente_mb";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
	...
	{
		$query="SELECT COUNT(*) AS n FROM Lotto_corrente_mb";
		$result=sqlsrv_query($conn,$query);
		if($result==FALSE)
		{
			$query=str_replace("'","*APICE*",$query);
