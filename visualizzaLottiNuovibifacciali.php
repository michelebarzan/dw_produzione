<?php
	...
	$pageName="Visualizza lotti nuovibifacciali";
	$appName="Produzione";
?>
<html>
	<head>
		...
		<script>
			function showTableLotti(id)
			{
				...
				xmlhttp.open("POST", "tabellaLottiNuovibifacciali.php?commessa=" + commessa, true);
				xmlhttp.send();
			}
			...
			function genera(n,id)
			{
				...
				xmlhttp.open("POST", "generaExcelLottiNuovibifacciali.php?id_lotto=" + id, true);
			...
			function svuotaLottoCorrenteTimeout()
			{
				...
				xmlhttp.open("POST", "svuotaLottoNuovobifaccialeCorrenteTimeout.php?", true);
				xmlhttp.send();
			}
			function modifica(riga,colonna,id)
			{
				...
				xmlhttp.open("POST", "aggiornaNoteNuovobifacciale.php?nomeColonna=" + nomeColonna + "&valore=" + valore + "&id=" + id, true);
				xmlhttp.send();
			}
			function erroreGenerabile(lotto,commessa)
			{
				...
				xmlhttp.open("POST", "erroreGenerabileNuovobifacciale.php?lotto=" + lotto + "&commessa=" + commessa, true);
				xmlhttp.send();
			}
			function controlloIntegritaLotti()
			{
				...
				$.post("controlloIntegritaLotti_nb.php",
				function(response, status)
				{
					if(status=="success")
					{
						if(response.indexOf("ok")>-1)
						{
							...
							({
								type: 'success',
								title: 'Controllo integrita lotti nuovibifacciali completato'
							})
							showTableLotti(0);
							removeCircleSpinner();
						}
						else
						{
							Swal.fire({
							  type: 'error',
							  title: 'Errore',
							  text: "Impossibile controllare l'integrita dei lotti nuovibifacciali"
							})
						}
					}
					else
		...
		<div id="container">
			<div id="content">
				...
				<div id="righe" class="righe">Lotti&nbspdella&nbspcommessa&nbsp
					...
					$queryCommessa2="SELECT DISTINCT commesse.commessa FROM commesse,lotti_nb WHERE lotti_nb.commessa=commesse.id_commessa";
