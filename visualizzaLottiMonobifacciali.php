<?php
	...
	$pageName="Visualizza lotti monobifacciali";
	$appName="Produzione";
?>
<html>
	<head>
		...
		<script>
			function showTableLotti(id)
			{
				...
				xmlhttp.open("POST", "tabellaLottiMonobifacciali.php?commessa=" + commessa, true);
				xmlhttp.send();
			}
			...
			function genera(n,id)
			{
				...
				xmlhttp.open("POST", "generaExcelLottiMonobifacciali.php?id_lotto=" + id, true);
			...
			function svuotaLottoCorrenteTimeout()
			{
				...
				xmlhttp.open("POST", "svuotaLottoMonobifaccialeCorrenteTimeout.php?", true);
				xmlhttp.send();
			}
			function modifica(riga,colonna,id)
			{
				...
				xmlhttp.open("POST", "aggiornaNoteMonobifacciale.php?nomeColonna=" + nomeColonna + "&valore=" + valore + "&id=" + id, true);
				xmlhttp.send();
			}
			function erroreGenerabile(lotto,commessa)
			{
				...
				xmlhttp.open("POST", "erroreGenerabileMonobifacciale.php?lotto=" + lotto + "&commessa=" + commessa, true);
				xmlhttp.send();
			}
			function controlloIntegritaLotti()
			{
				...
				$.post("controlloIntegritaLotti_mb.php",
				function(response, status)
				{
					if(status=="success")
					{
						if(response.indexOf("ok")>-1)
						{
							...
							({
								type: 'success',
								title: 'Controllo integrita lotti monobifacciali completato'
							})
							showTableLotti(0);
							removeCircleSpinner();
						}
						else
						{
							Swal.fire({
							  type: 'error',
							  title: 'Errore',
							  text: "Impossibile controllare l'integrita dei lotti monobifacciali"
							})
						}
					}
					else
		...
		<div id="container">
			<div id="content">
				...
				<div id="righe" class="righe">Lotti&nbspdella&nbspcommessa&nbsp
					...
					$queryCommessa2="SELECT DISTINCT commesse.commessa FROM commesse,lotti_mb WHERE lotti_mb.commessa=commesse.id_commessa";
